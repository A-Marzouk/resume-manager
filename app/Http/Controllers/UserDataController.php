...
//                   'photo'=>'required',
    ...
    public function saveBehanceProjects($projects)
    {
        ...
        foreach ($projects as $project) {
            ...
            $dist = "uploads/project" . $project->id . "Behance" . $user->id . ".png";
            if (!file_exists($dist)) {
                ...
                foreach ($modules as $module) {
                    if (isset($module->src)) {
                        if (strpos($module->src, 'embed') !== false) { // it is video link !
                            ...
                            $distMoreMedia = "uploads/behance_data/new_img" . $project->id . "Behance" . $user->id . $newName;
    ...
    public function dataFromLinkedIn()
    {
        ...
        try {
            ...
            $res = $client->request('POST', 'https://www.linkedin.com/oauth/v2/accessToken',
                ['form_params' =>
                    [
                        ...
                        'redirect_uri' => 'https://123workforce.com/freelancer/linkedin',
                        'code' => $_GET['code'] ?? ''
                    ]
                ]
            );
            ...
            $user = $client->request('GET', 'https://api.linkedin.com/v2/me?projection=(id,localizedLastName,pictureUrl,localizedFirstName,picture-url,profilePicture(displayImage~:playableStreams))&oauth2_access_token=' . $responseArr["access_token"]);
            $userArr = json_decode($user->getBody()->getContents());

            $image ='displayImage~';
            $image_url = $userArr->profilePicture->$image->elements[3]->identifiers[0]->identifier ;

            $currentUser = auth()->user();
            $userData = UserData::where('user_id', $currentUser->id)->first();
            $userData->name = $userArr->localizedFirstName . ' ' .$userArr->localizedLastName;
            $userData->photo = $image_url;
            $userData->save();
    ...
    public function saveAudio(Request $request)
    {
        ...
        if (isset($_FILES['file']) and !$_FILES['file']['error']) {
            ...
            $target_file = "uploads/audios/" . $fname;
    ...
    public function saveAudioForChat(Request $request)
    {
        if (isset($_FILES['file']) and !$_FILES['file']['error']) {
            ...
            $target_file = "storage/chat_shared_files/" . $fname;
