<template>
  <div>
    ...
    <transition-group name="list"
                      class="row">
      <invoice-component v-for="(invoice,index) in invoices"
                         v-bind:key="index"
    ...
    height: 15px;">
            ...
            <div style="color: #30323D;font-family: Roboto;"
                 v-show="invoice.notes != null">Notes : {{invoice.notes}}</div>
            <hr>
    ...
    <div class="pageSubHeading text-left"
         v-show="this.invoices.length < 1">
      Currently this client has no invoices.
    </div>
    <add-invoice-modal @invoiceAdded="addInvoice"
                       :toBeEditedInvoice="toBeEditedInvoice"></add-invoice-modal>
  </div>
</template>
<script>
export default {
