<template>
  <div>
    <transition-group name="list"
                      class="row">
      <job-post v-for="(job,index) in jobs"
                v-bind:key="index"
    ...
    height: 15px;">
        ...
        <button :id="'seeApplied'+job.id"
                class="btn btn-sm btn-outline-primary"
                @click="getAppliedFreelancers(job.id)">See who applied</button>
        <br />
        <div v-for="(data,index) in appliedFreelancers"
             v-bind:key="index">
    ...
    <br />
    <add-job-modal @jobAdded="addJobPost"
                   :toBeEditedJob="toBeEditedJob"></add-job-modal>
  </div>
</template>
<script>
export default {
